...
import calendar from "../../assets/icons/blog/can.svg";
...
const NewsSingle = ({ newsProps }) => {
  ...
  return (
    <section className="w-full grid grid-cols-[48%_125px_1fr] gap-0 py-[1.875rem] border-y border-[#D6D6D6]">
      ...
      >
        <figcaption
          className={`w-4/5 h-4/5 bg-blue91 flex items-center justify-center z-[2] relative
                    ${visibility} transition-opacity duration-300`}
        >
          <div
            className={`w-full h-full absolute top-0 left-0 bg-blue91 z-[1] ${visibility}`}
          ></div>
          <Link
            to={link}
            className="font-krub font-semibold text-base text-white z-[3] relative
                        hover:text-gold"
          >
            Read More
          </Link>
      ...
      <div className="w-full flex flex-col items-center text-center">
        ...
        <p className="font-krub font-medium text-base text-ptext">{month}</p>
      </div>
      <article className="w-full pl-[1.875rem] border-l">
        <h5
          className="font-rubik font-normal text-[1.5625rem] leading-[1.875rem] text-htext 
